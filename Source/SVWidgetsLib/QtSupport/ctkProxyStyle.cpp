...
=========================================================================*/
/*************************************************************************
* The License file can be found in SIMPL/Resources/ThirdParty/ctk.txt
*/
// Qt includes
...
{
  if(proxy->baseStyle() == style && style->proxy() == proxy)
  {
    return;
  }
...
void ctkProxyStyle::ensureBaseStyle() const
{
  ...
  QProxyStyle* proxyStyle = const_cast<QProxyStyle*>(qobject_cast<const QProxyStyle*>(this->proxy() ? this->proxy() : this));
...
// ----------------------------------------------------------------------------
void ctkProxyStyle::drawItemText(QPainter* painter, const QRect& rect, int flags, const QPalette& pal, bool enabled, const QString& text, QPalette::ColorRole textRole) const
{
  Q_D(const ctkProxyStyle);
  this->ensureBaseStyle();
...
{
  Q_D(const ctkProxyStyle);
  if(e->type() != QEvent::ParentChange && e->type() != QEvent::ChildRemoved && e->type() != QEvent::ChildAdded)
  {
    this->ensureBaseStyle();
  }
...
// ----------------------------------------------------------------------------
QIcon ctkProxyStyle::standardIconImplementation(StandardPixmap standardIcon, const QStyleOption* option, const QWidget* widget) const
{
  Q_D(const ctkProxyStyle);
  this->ensureBaseStyle();
...
// ----------------------------------------------------------------------------
int ctkProxyStyle::layoutSpacingImplementation(QSizePolicy::ControlType control1, QSizePolicy::ControlType control2, Qt::Orientation orientation, const QStyleOption* option,
