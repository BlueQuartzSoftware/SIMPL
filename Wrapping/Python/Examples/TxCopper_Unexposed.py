...
def txcopper_unexposed():
    ...
    err = orientationanalysispy.read_ctf_data(dca, data_container_name,
    ...
    # Conversion type may need to be enumeration ?
    err = orientationanalysispy.change_angle_representation(dca, 0, simpl.DataArrayPath(data_container_name,
                                                            "EBSD Scan Data", "EulerAngles"))
    if err < 0:
        print("ChangeAngleRepresentation ErrorCondition: %d" % err)
    ...
    err = itkimageprocessingpy.itk_image_writer(dca, sd.GetBuildDirectory() +
                                                "/Debug/Data/Output/TexturedCopper/IPF_Unexposed.png",
                                                simpl.DataArrayPath(data_container_name, "EBSD Scan Data",
                                                                    "IPF_Unexposed_001"), 0)
    if err < 0:
