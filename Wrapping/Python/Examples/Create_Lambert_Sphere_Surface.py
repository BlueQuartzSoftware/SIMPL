...
def runPythonicTest(dca):
  err = d3d.create_data_container(dca, "ImageDataContainer")
  if err < 0:
    print("DataContainer ErrorCondition: %d" % err)

  err = d3d.create_image_geometry(dca, "ImageDataContainer", simpl.IntVec3(101, 101, 1), simpl.FloatVec3(0,0,0), simpl.FloatVec3(1,1,1))
  if err < 0:
  ...
  tableData = list()
  tableData.append(newRow)
  err = d3d.create_attribute_matrix(dca, simpl.DataArrayPath("ImageDataContainer", "CellAttributeMatrix", ""), 3, simpl.DynamicTableData(tableData, ["0", "1", "2"], ["0"]))
  if err < 0:
    print("AttributeMatrix ErrorCondition: %d" % err)
    
  err = d3d.create_data_array(dca, simpl.ScalarTypes.UInt8, 1, simpl.DataArrayPath("ImageDataContainer", "CellAttributeMatrix", "ScalarValues"),
  ...
  err = orientationanalysis_pythonic.create_lambert_sphere(dca, sc.Hemisphere.Northern, simpl.DataArrayPath("ImageDataContainer","CellAttributeMatrix","ScalarValues"),
  ...
  err = d3d.data_container_writer(dca, sd.GetTestTempDirectory() + "/LambertSphere.dream3d", True, False)
  if err < 0:
