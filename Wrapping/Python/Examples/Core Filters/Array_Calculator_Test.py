...
def array_calculator_test():
    ...
    err = simplpy.array_calculator(dca, simpl.DataArrayPath("Small IN100", "EBSD Scan Data", ""), 
                                   "IPFColor*3.1415927*100", simpl.DataArrayPath("Small IN100", "EBSD Scan Data", "Output"),
                                   simpl.AngleUnits.Radians, simpl.ScalarTypes.UInt32)
    if err < 0:
        print("ArrayCalculator ErrorCondition %d" % err)

    # Write to DREAM3D file
    err = sc.WriteDREAM3DFile(sd.GetBuildDirectory()
                              + "/Debug/Data/Output/CoreFilterTests/SmallIN100_ArrayCalculator.dream3d", dca)
    if err < 0:
