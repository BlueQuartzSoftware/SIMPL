...
# This pipeline is designed to test the Convert Data filter
#
...
def convert_data_test():
    ...
    err = simplpy.convert_data(dca, simpl.NumericTypes.UInt8, "IPFColor_uint8",
                               simpl.DataArrayPath("Small IN100", "EBSD Scan Data", "IPFColor"))
    if err < 0:
        print("ConvertData ErrorCondition %d" % err)

    # Write to DREAM3D file
    err = sc.WriteDREAM3DFile(sd.GetBuildDirectory()
                              + "/Debug/Data/Output/CoreFilterTests/SmallIN100_ConvertData.dream3d", dca)
    if err < 0:
