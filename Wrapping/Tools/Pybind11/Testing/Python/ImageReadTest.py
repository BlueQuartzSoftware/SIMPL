...
from simpl import *
from simplutil import *
import imageio
import DataArrayTest
...
def ImageReadTest():
  ...
  tif = imageio.imread(GetDataDirectory() + "/Data/Image/slice_11.tif")    
  z = np.asarray(tif)
  ...
  dca = DataArrayTest.CreateDataContainerArray()

  # Create the DataContainer
  dc = DataArrayTest.CreateDataContainer("ImageDataContainer")
  dca.addDataContainer(dc)

  # Create an AttributeMatrix and add it to the DataContainer
  amShape = simpl.VectorSizeT([shape[1], shape[0], 1])
  cellAm = DataArrayTest.CreateAttributeMatrix("CellAttributeMatrix", simpl.AttributeMatrix.Type.Cell, amShape)
  ...
  # Write the file out
  err = DataArrayTest.WriteDREAM3DFile( GetTestTempDirectory() + "/ImageReadTest.dream3d", dca)
  assert err == 0
