...
* initialized.
*/
PySharedPtrClass<ScaleVolume> pybind11_init_SIMPLib_ScaleVolume(py::module &m, PySharedPtrClass<AbstractFilter>& parent)
{
  PySharedPtrClass<ScaleVolume> instance(m, "ScaleVolume", parent);
  instance
  .def(py::init([]()
    { 
      return ScaleVolume::New();
    }
))
  .def_static("New", &ScaleVolume::New)
