...
const char* name[] = {"dax.h", "yow.h", "boz.h", "zow.h", "foo.cpp", "foo.o", "bar.cpp", "bar.o", "libfoobar.a", "zig.cpp", "zig.o", "zag.cpp", "zag.o", "libzigzag.a", "killerapp"};
...
#define MAKE_EDGE_PAIR(node0, node1) QUuid::createUuidV5(uuid, QString::number(node0)), QUuid::createUuidV5(uuid, QString::number(node1))

#define MAKE_NODE(node0) QUuid::createUuidV5(uuid, QString::number(node0))
...
int main(int, char* [])
{
  ...
  Edge used_by[] = {Edge(MAKE_EDGE_PAIR(dax_h, foo_cpp)),     Edge(MAKE_EDGE_PAIR(dax_h, bar_cpp)),           Edge(MAKE_EDGE_PAIR(dax_h, yow_h)),          Edge(MAKE_EDGE_PAIR(yow_h, bar_cpp)),
                    Edge(MAKE_EDGE_PAIR(yow_h, zag_cpp)),     Edge(MAKE_EDGE_PAIR(boz_h, bar_cpp)),           Edge(MAKE_EDGE_PAIR(boz_h, zig_cpp)),        Edge(MAKE_EDGE_PAIR(boz_h, zag_cpp)),
                    Edge(MAKE_EDGE_PAIR(zow_h, foo_cpp)),     Edge(MAKE_EDGE_PAIR(foo_cpp, foo_o)),           Edge(MAKE_EDGE_PAIR(foo_o, libfoobar_a)),    Edge(MAKE_EDGE_PAIR(bar_cpp, bar_o)),
                    Edge(MAKE_EDGE_PAIR(bar_o, libfoobar_a)), Edge(MAKE_EDGE_PAIR(libfoobar_a, libzigzag_a)), Edge(MAKE_EDGE_PAIR(zig_cpp, zig_o)),        Edge(MAKE_EDGE_PAIR(zig_o, libzigzag_a)),
                    Edge(MAKE_EDGE_PAIR(zag_cpp, zag_o)),     Edge(MAKE_EDGE_PAIR(zag_o, libzigzag_a)),       Edge(MAKE_EDGE_PAIR(libzigzag_a, killerapp))};
  const std::size_t nedges = sizeof(used_by) / sizeof(Edge);

  typedef std::vector<QUuid> vecUuidType;

  typedef boost::adjacency_list<vecUuidType, vecUuidType, boost::bidirectionalS, boost::property<boost::vertex_name_t, std::string>> Graph;
...
//  typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS> Graph;
  ...
  // and the order with files that can be compiled in parallel
  {
    ...
    std::cout << "parallel make ordering, " << std::endl << "vertices with same group number can be made in parallel" << std::endl;
    {
